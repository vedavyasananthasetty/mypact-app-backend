...
postController.delete('/:id', verifyToken, async (req , res) => {
  try {
    ...
    if(!post){
      ...
      return res.status(500).json({msg:"You can delete only your posts."})
    } else {
...
postController.put('/togglelike/:id', verifyToken, async (req, res) => {
  try {
    ...
    //else add him in likes array

    if(post.likes.includes(currentUserId)){
      post.likes = post.likes.filter((id) => id !== currentUserId)
      await post.save()
      return res.status(200).json({msg: "Unliked the post"})
    } else {
      post.likes.push(currentUserId)
      await post.save()
      return res.status(200).json({msg: "Liked the post"})
    }
  } catch (error) {
    return res.status(500).json(error.message)
  }
